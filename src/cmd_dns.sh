...
# Linux: write dnsmasq config for .test domains, bind to 127.0.0.1 only
configure_dnsmasq_linux() {
    local conf
    if [ -d "/etc/dnsmasq.d" ]; then
        conf="/etc/dnsmasq.d/wp-test.conf"
        printf 'bind-interfaces\nlisten-address=127.0.0.1\naddress=/test/127.0.0.1\n' | sudo tee "$conf" > /dev/null
    else
        conf="/etc/dnsmasq.conf"
        echo "address=/test/127.0.0.1" | sudo tee -a "$conf" > /dev/null
    fi
}
...
cmd_dns_install() {
    ...
    if [ "$os" = "macos" ]; then
        ...
        configure_dnsmasq_linux
        echo -e "  ${GREEN}+${NC} Configured dnsmasq for .test domains"

        # Configure systemd-resolved to forward .test to dnsmasq (don't stop it!)
        if command -v systemctl &> /dev/null; then
            ...
            sudo systemctl restart dnsmasq || {
                echo -e "${YELLOW}Failed to start dnsmasq.${NC}"
